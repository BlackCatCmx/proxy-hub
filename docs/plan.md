...
- 管理代理保存时使用 `scheme+host+port+user+pass` 生成比较 key，不使用 `ID`。
...
```
┌─ proxy-hub ────────────────────────────────────[ 代理 | 设置 | 日志 ]──────┐
│ 分组: [ US-sk5 ] [ Fofa ] [ Test ]  │  [+ 新建分组] [ 重命名 ] [ 删除 ]  │
│ 当前分组: US-sk5                                                          │
│ [ 管理代理 ] [ 延迟测试 ] [ IP回显 ] [ 导出 ] [ 复制本组全部 ]          │
...
│ └─────────────────────────────────────────────────────────────────────┘   │
└────────────────────────────────────────────────────────────────────────────┘
延迟着色: ≤绿色延迟上限 绿  绿色延迟上限~红色延迟起点 黄  ≥红色延迟起点 红   错误: 红色，悬停显示全文
```
